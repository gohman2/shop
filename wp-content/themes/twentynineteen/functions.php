...
function twentynineteen_scripts() {
    wp_enqueue_script( 'boosta-jquery', 'https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js', array(), null, true );
    ...
    wp_enqueue_script('ajax-load', get_template_directory_uri() . '/js/ajax-load.js', 'boosta-jquery', 1, true);
    wp_localize_script( 'ajax-load', 'js_vars',
        array(
...
function posts_loader(){
    ...
    if ( wp_verify_nonce( $nonce, 'ajax-search-nonce' ) ) {
        if (!isset($_POST['postTotal']) || !isset($_POST['postOffset']) || !isset($_POST['postPerpage'])) {
            return;
        }
        $postTotal = $_POST['postTotal'];
        ...
        if( $postNot != "" ){
            $args['post__not_in'] = explode(",", $postNot);
        }
        $query = new WP_Query($args);
        $postTotal = $query->found_posts;
        $i = $postOffset + 1;
        $c = 1;
        if ($query->have_posts()):
        ...
        echo json_encode(array('content' => $content, 'postTotal' => $postTotal, 'postOffset' => $postOffset, 'postPerpage' => $postPerpage, 'more' => $more, 'i' => $i));
        exit;
    }
    die();
}